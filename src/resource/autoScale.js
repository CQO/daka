...
function getScale () {
  ...
  if ((autoScaleInfo.innerWidth / autoScaleInfo.innerHeight) > 1) {
    ...
    for (var index = 0; index < scaleListPC.length; index++) {
      ...
      autoScaleInfo.zoom = parseFloat(scale.toFixed(2))
      scaleBox.style.zoom = autoScaleInfo.zoom
      scaleBox.style.transformOrigin = 'center'
      scaleBox.style.position = 'absolute'
      scaleBox.style.left = (autoScaleInfo.innerWidth - autoScaleInfo.devisePCW * autoScaleInfo.zoom) / 2 + 'px'
      scaleBox.style.top = (autoScaleInfo.innerHeight - autoScaleInfo.devisePCH * autoScaleInfo.zoom) / 2 + 'px'
